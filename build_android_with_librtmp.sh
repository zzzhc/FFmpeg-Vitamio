...
if [ -z "$SSL" ]; then
  ...
  echo "Please run git clone git@github.com:yixia/OpenSSL-Vitamio.git, and run ndk-build ";
  exit 1
fi
...
for version in $TARGET; do
  ...
  case $version in
    ...
    vfp)
      ...
      EXTRA_LDFLAGS="-L$SSL/libs/armeabi -L$RTMPDUMP/libs/armeabi"
      SSL_OBJS=`find $SSL/obj/local/armeabi/objs/ssl $SSL/obj/local/armeabi/objs/crypto -type f -name "*.o"`
      RTMP_OBJS=`find $RTMPDUMP/obj/local/armeabi/objs/rtmp -type f -name "*.o"`
      ;;
    armv6)
      FFMPEG_FLAGS="--arch=arm \
        --disable-runtime-cpudetect \
    ...
    x86)
      ...
      EXTRA_LDFLAGS="-L$SSL/libs/x86 -L$RTMPDUMP/libs/x86"
      SSL_OBJS=`find $SSL/obj/local/x86/objs/ssl $SSL/obj/local/x86/objs/crypto -type f -name "*.o"`
      RTMP_OBJS=`find $RTMPDUMP/obj/local/x86/objs/rtmp -type f -name "*.o"`
      ;;
    mips)
      ...
      EXTRA_LDFLAGS="-L$SSL/libs/mips -L$RTMPDUMP/libs/mips"
      SSL_OBJS=`find $SSL/obj/local/mips/objs/ssl $SSL/obj/local/mips/objs/crypto -type f -name "*.o"`
      RTMP_OBJS=`find $RTMPDUMP/obj/local/mips/objs/rtmp -type f -name "*.o"`
      ;;
    *)
      FFMPEG_FLAGS=""
      EXTRA_CFLAGS=""
